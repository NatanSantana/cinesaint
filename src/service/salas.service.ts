...
export class SalasService {
  ...
  async cadastrarSala(sala: CreateSalaDto) {
    const salaExistente = await this.salasRepository.searchByNumero(
      sala.numero,
    );

    if (salaExistente !== null) {
      throw new ConflictException(
        'Já existe uma sala cadastrada com esse número!',
      );
    }
  ...
  async buscarSalaPorNumero(numero: number) {
    if (typeof numero !== 'number') {
      throw new BadRequestException(
        'Número da sala deve ser um número inteiro',
      );
    }

    if (numero < 1) {
      throw new BadRequestException(
        'Número da sala deve ser um número inteiro positivo',
      );
    }
