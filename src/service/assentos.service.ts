...
export class AssentosService {
    ...
    async registrarAssentos(assentos: CreateAssentosDto) {
        ...
        const assento = await this.assentosRepository.buscarAssentosByIdentificacao(assentos.identificacao);

        if (assento) {
            throw new ConflictException("Já existe um assento com essa identificação")
        }
    ...
    async atualizarAssentosManutencao(identificacao: string) {
        ...
        const atualizar = await this.assentosRepository.atualizarAssentosComoManutencao(identificacao);

        if (!atualizar) {
            throw new NotFoundException("Não foi possível encontrar o assento com essa identificação")
        }
    ...
    async atualizarAssentosComoLivres(identificacao: string) {
        ...
        const assentos = await this.assentosRepository.atualizarAssentosComoLivre(identificacao);
        if (assentos.count === 0) {
            throw new NotFoundException("Não foi possível encontrar um assento com essa identificação")
        }
